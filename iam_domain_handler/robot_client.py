...
class RobotClient:
    ...
    def run_skill(self, skill_name, skill_param):
        if self._running_skill_id is not None:
            assert self.get_skill_status(self._running_skill_id) in ('success', 'failure'), 'A skill is currently running!'

        self._running_skill_id = self._action_registry_client.register_action(skill_name, skill_param)

        # Make this request async so we don't wait until skill finishes
        def req_run_skill():
            self._run_skill_srv_proxy(self._running_skill_id)
