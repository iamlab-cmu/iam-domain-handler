...
class RobotServer:
    ...
    def __init__(self, skills_dict):
        ...
        for skill in self._skills_dict.values():
            ...
            or isinstance(skill, BaseOneStepQuatPoseDmpSkill)
    ...
    def _run_policy_on_robot(self, skill_id, init_state, policy, param, skill):
        if policy.cmd_type == CmdType.GRIPPER:
            ...
            return 1
        elif policy.cmd_type == CmdType.ONESTEPQUATPOSEDMP:

            rate = rospy.Rate(1 / policy.dt)
            t_step = 0
            self._fa.execute_quaternion_pose_dmp(policy.position_dmp_params, policy.quat_dmp_params, duration=policy.duration, goal_quat=self._fa.get_pose().quaternion, goal_quat_time=policy.duration, block=False)
